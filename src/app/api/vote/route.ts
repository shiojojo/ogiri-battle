...
export async function POST(req: NextRequest) {
  try {
    ...
    const { jokeId, type, userId, guestName } = body as { jokeId?: string; type?: string; userId?: string; guestName?: string };
    if (!jokeId || !type) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    if (!['ippon','waza','valid'].includes(type)) return NextResponse.json({ error: 'Bad type' }, { status: 400 });
